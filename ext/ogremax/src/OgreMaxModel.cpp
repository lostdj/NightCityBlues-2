...
CameraParameters* OgreMaxModel::LoadCamera(const TiXmlElement* objectElement)
{
    ...
    parameters->orthoWidth = OgreMaxUtilities::GetRealAttribute(objectElement, "orthoWidth", (Real)50);
    parameters->orthoHeight = OgreMaxUtilities::GetRealAttribute(objectElement, "orthoHeight", (Real)50);
    ...
    ) const
{
    ...
    //Handle node extra data
    if (!nodeParams.extraData.isNull() && (nodeParams.extraData->HasUserData() || nodeParams.extraData->HasNoteTracks()))
    {
    ...
    ) const
{
    ...
    //Set subentities
    OgreMaxUtilities::SetSubentities(entity, entityParams->subentities);
    
    //Set manual LODs
    OgreMaxUtilities::SetManualLods(entity, entityParams->manualLods);
    ...
    ) const
{
	...
	if (lightParams->direction == Vector3::ZERO)
		light->setDirection(lightParams->orientation * light->getDirection());
	else
    ...
    ) const
{
    ...
    if (cameraParams->projectionType == PT_ORTHOGRAPHIC)
        camera->setOrthoWindow(cameraParams->orthoWidth, cameraParams->orthoHeight);
    ...
    if (cameraParams->direction == Vector3::ZERO)
