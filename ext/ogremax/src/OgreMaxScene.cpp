...
void OgreMaxScene::LoadRenderTextures(const TiXmlElement* objectElement)
{
    ...
    while (renderTextureElement = OgreMaxUtilities::IterateChildElements(objectElement, renderTextureElement))
    {
        ...
        while ((childElement = OgreMaxUtilities::IterateChildElements(renderTextureElement, childElement)))
        {
            ...
            if (elementName == "colourBackground" || elementName == "backgroundColor")
...
void OgreMaxScene::LoadNode(const TiXmlElement* objectElement, SceneNode* parentNode)
{
    ...
    while ((childElement = OgreMaxUtilities::IterateChildElements(objectElement, childElement)))
    {
        ...
        {
            extraData.noteTracks = SharedPtr<NoteTracks>(new NoteTracks);
            OgreMaxUtilities::LoadNoteTracks(childElement, *extraData.noteTracks.get());
        }
        else if (elementName == "position")
    ...
    if (extraData.HasUserData() || extraData.HasNoteTracks())
    {
...
void OgreMaxScene::LoadShadows(const TiXmlElement* objectElement)
{
    ...
    String elementName;
    const TiXmlElement* childElement = 0;
    const TiXmlElement* shadowCameraElement = 0;
    while ((childElement = OgreMaxUtilities::IterateChildElements(objectElement, childElement)))
    {
        elementName = childElement->Value();
        ...
        {
            params.useDepthTexture = OgreMaxUtilities::GetBoolAttribute(objectElement, "useDepthTexture", params.selfShadow);
        ...
        {
            shadowCameraElement = childElement;          
            params.cameraSetup = OgreMaxUtilities::GetStringAttribute(shadowCameraElement, "type", "lispsm");
        }
    }
    ...
    else
    {
        ...
        if (shadowCameraElement != 0)
        {
            //Use the specified setup
            shadowCameraSetupPtr = ShadowCameraSetupPtr(LoadShadowCameraSetup(shadowCameraElement, params.cameraSetup));
        }
        else
        {
...
void OgreMaxScene::LoadEntity(const TiXmlElement* objectElement, const MovableObjectOwner& owner)
{
    ...
    String skeletonAnimationBlendMode = OgreMaxUtilities::GetStringAttribute(objectElement, "skeletonAnimationBlendMode");
    parameters.skeletonAnimationBlendMode = OgreMaxUtilities::ParseSkeletonAnimationBlendMode(skeletonAnimationBlendMode);
    ...
    while ((childElement = OgreMaxUtilities::IterateChildElements(objectElement, childElement)))
    {
        ...
        else if (elementName == "noteTracks")
        {
            ...
            OgreMaxUtilities::LoadManualLods(childElement, parameters.manualLods);
    ...
    if (skeleton != 0)
        skeleton->setBlendMode(parameters.skeletonAnimationBlendMode);

    objectExtraData->object = entity;

    //Set subentities
    OgreMaxUtilities::SetSubentities(entity, parameters.subentities);
    
    //Set manual LODs
    OgreMaxUtilities::SetManualLods(entity, parameters.manualLods);
...
void OgreMaxScene::LoadLight(const TiXmlElement* objectElement, const MovableObjectOwner& owner)
{
    ...
    while ((childElement = OgreMaxUtilities::IterateChildElements(objectElement, childElement)))
    {
        ...
        if (elementName == "colourDiffuse")
			...
			light->setDirection(OgreMaxUtilities::LoadRotation(childElement) * light->getDirection());
...
void OgreMaxScene::LoadCamera(const TiXmlElement* objectElement, const MovableObjectOwner& owner)
{
    ...
    String projectionType = OgreMaxUtilities::GetStringAttribute(objectElement, "projectionType", "perspective");
    Real orthoWidth = OgreMaxUtilities::GetRealAttribute(objectElement, "orthoWidth", (Real)50);
    Real orthoHeight = OgreMaxUtilities::GetRealAttribute(objectElement, "orthoHeight", (Real)50);
...
ShadowCameraSetup* OgreMaxScene::LoadShadowCameraSetup(const TiXmlElement* objectElement, const String& type)
{
    String typeLower = type;
    StringUtil::toLowerCase(typeLower);

    if (typeLower.empty() || typeLower == "uniform")
    ...
    else if (typeLower == "uniformfocused")
    {
        ...
        setup->setUseAggressiveFocusRegion(OgreMaxUtilities::GetBoolAttribute(objectElement, "useAggressiveFocusRegion", true));
        return setup;
    }
    else if (typeLower == "lispsm")
    {
        ...
        setup->setUseAggressiveFocusRegion(OgreMaxUtilities::GetBoolAttribute(objectElement, "useAggressiveFocusRegion", true));
        setup->setUseSimpleOptimalAdjust(OgreMaxUtilities::GetBoolAttribute(objectElement, "useSimpleOptimalAdjust", true));
        float optimalAdjustFactor = 0;
        if (!setup->getUseSimpleOptimalAdjust())
            optimalAdjustFactor = OgreMaxUtilities::GetRealAttribute(objectElement, "optimalAdjustFactor", (Ogre::Real).1);
        setup->setOptimalAdjustFactor(optimalAdjustFactor);
        setup->setCameraLightDirectionThreshold(Radian(OgreMaxUtilities::GetRealAttribute(objectElement, "lightDirectionThreshold", (Ogre::Real)0.45102624)));
        return setup;
    }
    else if (typeLower == "pssm")
    {
        PSSMShadowCameraSetup* setup = new PSSMShadowCameraSetup;
        setup->setUseAggressiveFocusRegion(OgreMaxUtilities::GetBoolAttribute(objectElement, "useAggressiveFocusRegion", true));
        setup->calculateSplitPoints
            (
            OgreMaxUtilities::GetIntAttribute(objectElement, "splitCount", 3),
            OgreMaxUtilities::GetRealAttribute(objectElement, "splitNearDistance", (Ogre::Real)100.0),
            OgreMaxUtilities::GetRealAttribute(objectElement, "splitFarDistance", DEFAULT_MAX_DISTANCE),
            OgreMaxUtilities::GetRealAttribute(objectElement, "splitReduction", (Ogre::Real).95)
            );
        setup->setSplitPadding(OgreMaxUtilities::GetRealAttribute(objectElement, "splitPadding", (Ogre::Real)1.0));
        return setup;
    }
    else if (typeLower == "planeoptimal")
    {
        ...
        *(Plane*)this->shadowOptimalPlane = OgreMaxUtilities::GetPlaneAttributes(objectElement, upVector.x, upVector.y, upVector.z, 0);
    ...
    OGRE_EXCEPT
        ...
        Exception::ERR_INVALIDPARAMS,
	    errorMessage.str(),
	    "OgreMaxScene::LoadShadowCameraSetup"
        );
}
